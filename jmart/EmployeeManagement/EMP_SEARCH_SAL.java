...
public class EMP_SEARCH_SAL {
    public static void EMP_SEARCH_SAL(){
        Scanner input = new Scanner(System.in);
        try {
            Connection connection = Connection_Provider.createConnection();
            Statement statement=connection.createStatement();
            System.out.println("Enter the salary : ");
            String sersal = input.next();
            System.out.println("Enter the operation :: = || > || < || <= || >= ");
            String operation = input.next();
            System.out.println("Enter the Limit of results you want to see ");
            String res_limit = input.next();

            String query_statement= "SELECT * FROM Employees WHERE salary "+operation+sersal+" ORDER BY salary,id limit 0 ,"+res_limit+";";
            ...
            String count_query = "SELECT COUNT(id) FROM Employees WHERE salary "+operation+sersal+";";
            ...
            String sum_query = "SELECT SUM(salary) FROM Employees WHERE salary "+operation+sersal+";";
            ...
            String avg_query = "SELECT AVG(salary) FROM Employees WHERE salary "+operation+sersal+";";
            System.out.println(sum_query);
